...
{
	UINT8 response_data[8]= {0x04,0x74,0x20,0x08,0x04,0x00,0x00,0x00};//固定格式的回应，2K当做一个包(stm32f302必须2K为最低，因为flash一页为2K)
...
void upgrade_transferData(device_info_t *dev,u8 *buf, u16 len)
{
	...
	//	Upflag = 0;
	  ...
	  if(0 == Update_WriteAndCheck(dev,(u16*)&buf[4],curBagSeq,len-4))//len减去包编号占一个字节
