...
static irqreturn_t mxc_hdmi_hotplug(int irq, void *data)
{
	...
	if (intr_stat & HDMI_IH_PHY_STAT0_HPD) {
		...
		schedule_delayed_work(&(hdmi->hotplug_work), msecs_to_jiffies(20));//20
	}
...
static void mxc_hdmi_setup(struct mxc_hdmi *hdmi, unsigned long event)
{
	...
	hdmi->fbi->var.xres = 1280;
	hdmi->fbi->var.yres = 720;
	hdmi->fbi->var.pixclock = 13468;
	hdmi->fbi->var.left_margin = 220;
	hdmi->fbi->var.right_margin = 110;
	hdmi->fbi->var.upper_margin = 20;
	hdmi->fbi->var.lower_margin = 5;
	hdmi->fbi->var.hsync_len = 40;
	hdmi->fbi->var.vsync_len = 5;
	hdmi->fbi->var.sync = 3 ;
	hdmi->fbi->var.vmode = 32;
