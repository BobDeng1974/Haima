...
历史: 1. 2016/12/29, 初稿
	  ...
	  24. 2018/8/27，增加了枚举EN_TSH_APP_CLOSE;  
	                 增加了枚举EN_TSH_SDCARD_STATE;
                     添加了字段EN_TSH_FIELD_INFORM_CLOSEAPP.

      25. 2018/11/1，枚举EN_TSH_VEHICLE_EVENT增加新成员EN_TSH_VEHICLE_EVENT_CAR_FIRE
                     枚举EN_TSH_VIEWING_TYPE增加新成员EN_TSH_VIEW_DVR_HOME_PAGE
      26. 2018/11/14,枚举EN_TSH_MSG_TYPE增加新成员EN_TSH_MSG_UPDATE 
                     增加了字段EN_TSH_FIELD_UPDATE_MCUFILEDATA,EN_TSH_FIELD_UPDATE_MCUQUERYSTATE
                     增加了数据结构stUpateFileData_t,stUpateDateFeekback_t,stUpateDateState_t
	  27. 2018/11/21,增加了字段EN_TSH_FIELD_INFORM_MCURESTART
	                 增加枚举EN_TSH_MCU_RESTART
					 增加了数据结构stMCUVersion_t
      28. 2018/12/4, 枚举EN_TRACKLINE_TYPE增加新成员
...
typedef enum {
    ...
    EN_TSH_MSG_UPDATE           = 0x0009, // 更新MCU数据消息（内部App-->外部App）
...
enum {
    ...
    EN_TSH_FIELD_RECORDING                      = 0x03f2, // 录像(stRecording_t)
    ...
    EN_TSH_FIELD_EXITCALIBRATION                = 0x03fe, // 退出标定(NULL)
    EN_TSH_FIELD_HANDLECALIPICTURE              = 0x03ff, // 查询或者清除标定截图(stHandleCaliPicture_t)
    // inform  外部App使用
    // 需要在回调函数中进行解包
    ...
    EN_TSH_FIELD_INFORM_CLOSEAPP                = 0xfff4, // 通知关闭应用（uint32_t）值参考EN_TSH_APP_CLOSE
    EN_TSH_FIELD_INFORM_POEWRSTATE              = 0xfff3, // 通知电源状态（uint32_t）值参考EN_TSH_POWER_STATE
    EN_TSH_FIELD_UPDATE_MCUFILEDATA             = 0xfff2, // 更新MCU数据(stUpateFileData_t)
    EN_TSH_FIELD_UPDATE_MCUQUERYSTATE           = 0xfff1, // 查询更新MCU状态(stUpateFileData_t)
    EN_TSH_FIELD_INFORM_MCURESTART              = 0xfff0, // 通知MCU升级重启（uint32_t）值参考EN_TSH_MCU_RESTART
    EN_TSH_FIELD_INFORM_ADASFEATURES            = 0xffef, // 通知Adas功能(stAdasFeatures_t)  值参考EN_TSH_ADAS_TYPE
    EN_TSH_FIELD_INFORM_COLORPARAM              = 0xffee, // 通知调节亮度参数功能(stColorParam_t)  
...
typedef enum {
	...
	EN_TSH_VIEW_2D_FRONT_FULL                          = 33,//前镜头全屏幕画面
	EN_TSH_VIEW_2D_BACK_FULL                           = 34,//后镜头全屏幕画面
	EN_TSH_VIEW_3D_START_360ROTATE                     = 35,//开始3D环视
	EN_TSH_VIEW_3D_TERMINATE_360ROTATE				   = 36,//停止3D环视
	EN_TSH_VIEW_2D_LR_B_ACCOFF                         = 37,//ACC OFF 两侧视角
	EN_TSH_VIEW_2D_FRONT_WIDE                          = 38,//前广角
	EN_TSH_VIEW_2D_BACK_WIDE                           = 39,//后广角
	EN_TSH_VIEW_2D_LR_F_FULL                           = 40,//前轮两侧画面
	EN_TSH_VIEW_2D_LR_B_FULL                           = 41,//后轮两侧画面

	EN_TSH_VIEW_DVR_HOME_PAGE                          = 128,//显示DVR
...
typedef enum {
	...
	EN_TSH_VEHICLE_EVENT_CAR_FIRE                           = 41, // 汽车打火
...
// 主机状态
typedef enum {
	EN_TSH_SDCARD_STATE_NORMAL		                = 0,    //正常
	EN_TSH_SDCARD_STATE_NOSDCARD                    = 1,	//未安装
	EN_TSH_SDCARD_STATE_ERROR                       = 2, 	//故障
	EN_TSH_SDCARD_STATE_NOTFORMAT                   = 3, 	//不能被格式化 
	EN_TSH_SDCARD_STATE_INSUFFICIENT                = 4, 	//内存不足 
	EN_TSH_SDCARD_STATE_EMERGENCY_VIDEO_AREA_FULL   = 5, 	//紧急视频区域已满 
	EN_TSH_SDCARD_STATE_PHOTO_AREA_FULL             = 6 	//图片区满 
}EN_TSH_SDCARD_STATE;
...
typedef enum {
    ...
    EN_TSH_RADAR_STATE_DANGEROUS    = 6, // 危险
}EN_TSH_RADAR_STATE;
...
typedef enum {
    ...
    EN_TSH_CALIBRATION_TRIANGLE        = 3  // 三角标定
...
typedef enum {
    ...
    EN_TRACKLINE_TYPE_MERGER_FRONT      = 9, // 拼接区前轨迹线
    EN_TRACKLINE_TYPE_MERGER_REAR       = 10, //拼接区后轨迹线
}EN_TRACKLINE_TYPE;
...
typedef enum {
    EN_TSH_AVM_CLOSE     = 0,
    EN_TSH_DVR_CLOSE     = 0x00000001,
}EN_TSH_APP_CLOSE;

// 查询或者清除标定截图
typedef enum {
	EN_TSH_UNKNOWN_CALI_PICTURE					 = 0,	 //未知标定截图
	EN_TSH_QUERY_CALI_PICTURE		             = 1,    //查询标定截图
	EN_TSH_CLEAE_CALI_PICTURE                    = 2,	//清除标定截图
}EN_TSH_HANDLE_CALI_PICTURE;

typedef struct HandleCaliPicture {
    uint32_t nHandleType;     // 标定类型
}__attribute__((packed)) stHandleCaliPicture_t;

typedef struct UpateFileDate {
    char *mData;
    uint32_t mLen;
}__attribute__((packed)) stUpateFileData_t;
typedef struct UpateDateFeekback {
    uint32_t SerialNumber;
    uint32_t FeekbackResult;
}__attribute__((packed)) stUpateDateFeekback_t;
typedef struct UpateDateState {
    uint32_t state;
    uint32_t error;
}__attribute__((packed)) stUpateDateState_t;
typedef struct MCUVersion {
    uint32_t nHighV; // 主版本号，十进制
    uint32_t nLowV; // 次版本号，十进制
}__attribute__((packed)) stMCUVersion_t;
typedef enum {
	EN_MCU_RESTART_NONE              = 0,
    EN_MCU_RESTART_BOOTLOADER        = 0x00000001,
    EN_MCU_RESTART_POWEROFF          = 0x00000002,
}EN_TSH_MCU_RESTART;
typedef struct ColorParam {
	uint32_t nColorParam1;
	uint32_t nColorParam2;
	uint32_t nColorParam3;
	uint32_t nColorParam4;
}__attribute__((packed)) stColorParam_t;
