...
历史: 1. 2016/12/29, 初稿
      ...
      23. 2018/8/27，增加了内部调用API：
            void TshComm_InformCloseAPP(uint32_t CloseAPP)；
			void TshComm_SetSDCardState(uint32_t SDCardState);
		    增加了外部调用API函数：
			uint32_t TshComm_GetSDCardState();
	  24. 2018/10/11，增加了内部调用API
            void TshComm_SetCaliResult(uint32_t state);
            增加了外部调用API函数：
            uint32_t TshComm_GetCaliResult();
      25. 2018/10/19，增加了外部调用API函数：
            void TshComm_QueryOrClearCaliPicture(uint32_t type);
	  26. 2018/11/1，增加了内部调用API：
            void TshComm_InformPowerState(uint32_t PowerState);
      27. 2018/11/14,增加了内部调用API
            void TshComm_UpateMCUFileData(stUpateFileData_t *UpdateDate);
            void TshComm_UpateMCUQueryState (stUpateFileData_t *UpdateDate);
            增加了外部调用API函数：
			void TShComm_SetUpateDateFeekback(stUpateDateFeekback_t UpateDateFeekback);
            void TShComm_SetUpateDateState(stUpateDateState_t UpateDateState);
            void TShComm_GetUpateDateFeekback(stUpateDateFeekback_t* UpateDateFeekback); 
            void TShComm_GetUpateDateState(stUpateDateState_t* UpateDateState);
	  28. 2018/11/21,增加了内部调用API
	        void TshComm_InformMCURestart(uint32_t Type); 
			void TShComm_GetMCUVersion(stMCUVersion_t* MCUVersion);
			增加了外部调用API函数：
			void TShComm_SetMCUVersion(stMCUVersion_t MCUVersion);
	  29. 2018/12/29,增加了内部调用API
	        float TshComm_GetVehicleSteerAngle()
			增加了外部调用API函数：
			void TshComm_SetVehicleSteeringAngle(float angle)
*/
...
// 功能：获取SDCARD状态
// 返回：参考枚举类型EN_TSH_SDCARD_STATE
uint32_t TshComm_GetSDCardState();

// 功能：退出标定
void TshComm_ExitCalibration(uint32_t type);

// 功能：获取标定的状态
uint32_t TshComm_GetCaliResult();

// 功能：查询或者清除标定截图
// type：参考枚举类型EN_TSH_HANDLE_CALI_PICTURE
void TshComm_QueryOrClearCaliPicture(uint32_t type);
...
// 功能：通知关闭应用
// 参数CloseAPP：取值参考枚举EN_TSH_APP_CLOSE
void TshComm_InformCloseAPP(uint32_t CloseAPP);

// 功能：通知电源状态
// 参数PowerState：取值参考枚举EN_TSH_POWER_STATE
void TshComm_InformPowerState(uint32_t PowerState);
...
// 功能：通知MCU升级重启
// 参数Type：取值参考枚举EN_TSH_MCU_RESTART
void TshComm_InformMCURestart(uint32_t Type);
...
// 功能：设置SDCARD状态
// SDCardState：参考枚举类型EN_TSH_SDCARD_STATE
void TshComm_SetSDCardState(uint32_t SDCardState);

// 功能：设置标定的结果
void TshComm_SetCaliResult(uint32_t Result);
...
// 功能：通知Adas功能
// 参数adas：取值参考枚举EN_TSH_ADAS_TYPE
void TshComm_InformAdasFeatures(stAdasFeatures_t adas);

// 功能：通知调节亮度参数功能
// 参数Color
void TshComm_InformColorParam(stColorParam_t Color);
