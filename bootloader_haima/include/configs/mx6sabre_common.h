...
video=mxcfb1:off video=mxcfb2:off video=mxcfb3:off fbmem=10M,28M vmalloc=512M  root=${mmcroot} rootfstype=ext4\0 "  \
		"root=${mmcroot} \0" \
	"loadbootscript=" \
...
#define CONFIG_EXTRA_ENV_SETTINGS \
		...
		"smp=" CONFIG_SYS_NOSMP "\0"\
		"mmcargs=setenv bootargs console=${console},${baudrate} ${smp} \
	video=mxcfb0:dev=ldb,1280x720M@60,if=RGB24,bpp=32 ldb=sin0	\
	video=mxcfb1:off video=mxcfb2:off video=mxcfb3:off fbmem=10M,28M vmalloc=512M  root=${mmcroot} rootfstype=ext4\0 "	\
			"root=${mmcroot}\0" \
		"loadbootscript=" \
			"fatload mmc ${mmcdev}:${mmcpart} ${loadaddr} ${script};\0" \
